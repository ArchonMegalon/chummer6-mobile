...
| M12 Play-shell completion depth (post-closure) | in_progress | 90% | 2026-04-10 | medium | Browser transport, event-log persistence, offline resume, and observer-lane read-mostly shell posture remain closed, and post-closure role-shell completion gates are now explicit and verify-enforced; the latest additive depth slices now prove player/GM capability leakage cannot bleed into observer bootstrap or resume payloads, authorization denials preserve stored replay context instead of blanking the shell, workspace-lite continuity stays role-correct for both observer and GM follow-through, the shell now binds decision/update/support/restore/role links to projection-backed action copy instead of generic anchors, cache-pressure decision notices now reuse live support-next-step wording instead of a generic support CTA, and critical rejoin/continue/support command surfaces now ship with measurable low-noise route guidance, with remaining scope limited to sustained release proof refresh and any future role-depth regressions opened by new shell surfaces. |
...
| WL-027 | queued | P0 | Improve mobile UX continuity by adding explicit reconnect-and-recover state copy for disconnect, role change, and observer transitions. | agent | This slice reduces session confusion for players/GM by making resume intent, loss, and recovery options explicit. Depends on `core/WL-200` and `design/DR-120` before kickoff. Acceptance checks: (1) `METRICS_AND_SLOS.yaml` `session_continuity` and `campaign_and_dossier_continuity` pass for reconnect/rejoin flows; (2) runbook-ready recovery scripts are published in `design/DR-124`; (3) stale/offline/blocked states present next-safe-action copy with no silent rollback. |
...
| WL-029 | queued | P2 | Add empty-state onboarding and one-tap recovery pathways for no-session, no-campaign, and post-failure mobile flows. | agent | This slice prevents users from stalling on empty screens and improves first-resume success. Depends on `mobile/WL-028`, `design/DR-128`, and `design/DR-129`. Acceptance checks: (1) all entry states render one recommended next action; (2) retry/cancel/restore states are copy-consistent with `design/DR-126`; (3) no mobile recovery flow has unsupported action labels or locale mismatch. |
| WL-030 | queued | P2 | Add decision receipts for long-running shell actions (rejoin, quick actions, resume) so users can understand what was retried, skipped, or deferred. | agent | This slice reduces uncertainty when play loops include long-running operations. Depends on `mobile/WL-029` and `core/WL-202`; must include no-loss outcomes for resumable operations. Acceptance checks: (1) every long-running action records one visible progress/resume state; (2) visible copy aligns with `design/DR-129`; (3) support handoff points to one canonical escalation path. |
