...
| WL-024 | queued | P1 | Prove `D1` at the transport seams: mobile APIs must consume session semantics from canonical core contracts and publish role-shell replay state through transport adapters only. | agent | Add end-to-end checks in `scripts/ai/verify.sh` for `Chummer.Play.Contracts` consumption boundaries and remove any local semantic expansion in UI adapters. |

## Current repo truth

- Repo-local live queue: includes new D1 semantics follow-through rows pending transport-semantics evidence.
