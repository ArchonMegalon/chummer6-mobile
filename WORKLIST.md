...
| M13 Local-first sync and installable hardening depth | done | 100% | 2026-04-11 | high | Post-release hardening depth is now executable: offline queue recovery ergonomics, installable cache-pressure telemetry/UX clarity, and replay-safe resume fallback criteria are formalized as `TG-M13-*` gates and enforced by `scripts/ai/verify.sh`. |
| M14 Player-vs-GM completion depth | done | 100% | 2026-04-11 | high | Role-specific acceptance truth is now executable: player-versus-GM action guidance separation and observer handoff posture are formalized as `TG-M14-*` gates and enforced by `scripts/ai/verify.sh`. |
...
| WL-031 | done | P1 | Materialize post-M12 local-first sync/installable hardening depth with executable acceptance criteria and release-proof checks. | agent | Closed 2026-04-11 from auditor feedback (`project.uncovered_scope`): added explicit `M13` acceptance truth in `docs/PLAY_RELEASE_SIGNOFF.md`, published `TG-M13-OQ`, `TG-M13-IP`, `TG-M13-RF`, and `TG-M13-RP`, and wired `scripts/ai/verify.sh` plus release-proof materialization checks to enforce offline queue recovery, installable cache-pressure clarity, and replay-safe resume fallback behavior. |
| WL-032 | done | P1 | Complete milestone coverage for deeper player-vs-GM product completion with role-specific acceptance truth. | agent | Closed 2026-04-11 from auditor feedback (`project.milestone_coverage_incomplete`): added explicit `M14` acceptance truth in `docs/PLAY_RELEASE_SIGNOFF.md`, published `TG-M14-PL`, `TG-M14-GM`, `TG-M14-OB`, and `TG-M14-RP`, and wired `scripts/ai/verify.sh` to enforce role-specific completion evidence and closure traceability. |
...
## M13 Truth-Gate Registry

| Gate ID | Milestone | ETA Target (UTC) | Status | Completion Truth |
|---|---|---|---|---|
| TG-M13-OQ | M13 | 2026-04-11 | done | Offline queue recovery ergonomics remain executable: stale-lineage and malformed-envelope recovery paths reject unsafe mutations while preserving replay continuity through `VerifyOfflineQueueRejectsMalformedSessionEnvelopeAsync`, `VerifyOfflineQueueRejectsStaleLineageAsync`, and `VerifySyncPrefixAcknowledgementAsync`. |
| TG-M13-IP | M13 | 2026-04-11 | done | Installable cache-pressure telemetry and UX clarity remain executable: bounded cache-pressure posture plus caution/follow-through copy stay regression-backed through `VerifyCachePressureBudgetContractAsync`, `VerifyCachePressureDecisionNoticeUsesSupportNextActionCopy`, `VerifyCampaignWorkspaceLiteProjectionPromotesContinuitySummary`, and `VerifyIndexShellBindsContextualActionLabelsAsync`. |
| TG-M13-RF | M13 | 2026-04-11 | done | Replay-safe resume fallback remains executable under runtime-bundle drift: resume/reconnect fallback logic preserves stored lineage and role-concrete continuity routes through `VerifyResumePreservesLedgerWhenRuntimeBundleDriftsAsync`, `VerifyResumeNormalizesCheckpointToLedgerLineageAsync`, `VerifyReconnectLineageTransitionContinuityAsync`, and `VerifyResumeAndWorkspaceLiteRoutesStayRoleConcreteAsync`. |
| TG-M13-RP | M13 | 2026-04-11 | done | Ongoing release proof for hardening depth is explicit and enforceable: `docs/PLAY_RELEASE_SIGNOFF.md` now defines `M13` hardening criteria and cadence, and `scripts/ai/verify.sh` plus `scripts/materialize_mobile_local_release_proof.py` enforce those references. |

## M14 Truth-Gate Registry

| Gate ID | Milestone | ETA Target (UTC) | Status | Completion Truth |
|---|---|---|---|---|
| TG-M14-PL | M14 | 2026-04-11 | done | Player-lane completion depth remains role-specific and actionable: player quick actions stay player-safe under capability leakage, denied cross-role attempts preserve replay context, and workspace-lite posture/follow-through copy remains player-concrete through `VerifyRoleBoundarySurvivesCapabilityLeakageAsync`, `VerifyQuickActionRejectsCrossRoleAuthorizationAsync`, `VerifyDeniedQuickActionsPreserveStoredReplayStateAsync`, and `VerifyCampaignWorkspaceLiteProjectionPromotesContinuitySummary`. |
| TG-M14-GM | M14 | 2026-04-11 | done | GM-lane completion depth remains role-specific and actionable: GM action guidance stays separated from player quick actions, route ownership remains role-concrete across resume/workspace/restore/onboarding surfaces, and workspace-lite GM operations posture stays explicit through `VerifyBootstrapRoleShellEntryPointsAsync`, `VerifyRoleBoundarySurvivesCapabilityLeakageAsync`, `VerifyResumeAndWorkspaceLiteRoutesStayRoleConcreteAsync`, and `VerifyCampaignWorkspaceLiteProjectionPreservesObserverAndGmRoleDepth`. |
| TG-M14-OB | M14 | 2026-04-11 | done | Observer handoff coherence remains explicit and read-mostly: observer bootstrap/resume posture stays isolated from player/GM action lanes while observer-transition recovery and role follow-through guidance stay projection-backed through `VerifyObserverBootstrapAndResumeStayReadMostlyAsync`, `VerifyResumeAndWorkspaceLiteRoutesStayRoleConcreteAsync`, `VerifyCampaignWorkspaceLiteProjectionPreservesObserverAndGmRoleDepth`, and `VerifyIndexShellBindsContextualActionLabelsAsync`. |
| TG-M14-RP | M14 | 2026-04-11 | done | Ongoing release proof for role-depth completion is explicit and enforceable: `docs/PLAY_RELEASE_SIGNOFF.md` now defines `M14` role-specific criteria and cadence, and `scripts/ai/verify.sh` enforces those criteria plus `WL-032` closure traceability. |

## Current repo truth

- Repo-local live queue: `WL-031` and `WL-032` are now closed with explicit `M13` and `M14` truth gates and verifier enforcement.
