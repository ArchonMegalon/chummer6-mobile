...
| M13 Local-first sync and installable hardening depth | queued | 0% | 2026-04-26 | medium | Post-release depth slice for offline queue recovery ergonomics, installable cache-pressure telemetry/UX clarity, and replay-safe resume fallback criteria is now backlog-owned by `WL-031`. |
...
| WL-031 | queued | P1 | Materialize post-M12 local-first sync/installable hardening depth with executable acceptance criteria and release-proof checks. | agent | Opened 2026-04-11 from auditor feedback (`project.uncovered_scope`): convert remaining local-first sync + installable hardening scope into concrete implementation and regression tasks, including offline replay recovery behavior under cache pressure and explicit installable-shell fallback guidance criteria. |
...
- Repo-local live queue: `WL-031` and `WL-032` are queued as post-M12 depth work so uncovered mobile scope and milestone coverage stay executable instead of implicit.
