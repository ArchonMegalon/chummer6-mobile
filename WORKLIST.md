...
| M12 Play-shell completion depth (post-closure) | in_progress | 70% | 2026-04-10 | medium | Browser transport, event-log persistence, offline resume, and observer-lane read-mostly shell posture remain closed, and post-closure role-shell completion gates are now explicit and verify-enforced; the latest additive depth slice now proves player/GM capability leakage cannot bleed into observer bootstrap or resume payloads, with remaining scope limited to sustained release proof refresh and further role-depth assertions. |
...
| TG-M12-OB | M12 | 2026-04-10 | done | Observer-shell completion depth is explicit: bootstrap and resume payloads keep the lane read-mostly, expose observer-owned shell metadata, and reject inherited player/GM action posture through `VerifyObserverBootstrapAndResumeStayReadMostlyAsync` plus `VerifyRoleBoundarySurvivesCapabilityLeakageAsync`. |
