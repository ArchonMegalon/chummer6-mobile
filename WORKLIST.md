...
| M14 Player-vs-GM completion depth | queued | 0% | 2026-05-03 | medium | Post-release depth slice for role-specific completion polish, GM-versus-player action guidance separation, and observer handoff clarity is now backlog-owned by `WL-032`. |
...
| WL-032 | queued | P1 | Complete milestone coverage for deeper player-vs-GM product completion with role-specific acceptance truth. | agent | Opened 2026-04-11 from auditor feedback (`project.milestone_coverage_incomplete`): define and execute role-depth completion criteria for player versus GM shells (plus observer handoff coherence), with ETA/completion truth and verifier-backed closure evidence. |
...
- Repo-local live queue: `WL-031` is now closed with explicit `M13` hardening truth gates and verifier enforcement; `WL-032` remains queued for deeper role-specific completion depth.
